...
// PostgresLifecycleStore implements pkg.LifecycleStore for PostgreSQL.
type PostgresLifecycleStore struct {
	db platform.DB
}

// NewPostgresLifecycleStore returns a LifecycleStore backed by PostgreSQL.
func NewPostgresLifecycleStore(db platform.DB) *PostgresLifecycleStore {
	return &PostgresLifecycleStore{db: db}
}

func (s *PostgresLifecycleStore) RecordPlan(ctx context.Context, plan *pkg.LifecyclePlan) error {
	if plan == nil {
...
func (s *PostgresLifecycleStore) RecordWakeFacts(ctx context.Context, sessionID string, facts *pkg.WakeFacts) error {
	if facts == nil {
		return fmt.Errorf("storage: wake facts are nil")
	}

	planRow := s.db.QueryRowContext(ctx,
		`SELECT id FROM lifecycle_plans WHERE session_id = $1 ORDER BY resolved_at DESC LIMIT 1`,
		sessionID,
	)
	var planID string
...
func (s *PostgresLifecycleStore) RecordManifest(ctx context.Context, manifest *pkg.AssemblyManifest) error {
	if manifest == nil {
...
func (s *PostgresLifecycleStore) RecordDisclosure(ctx context.Context, sessionID, policyPath, policyHash, previousHash, changesSummary, policySource, policySnapshot string) error {
...
func (s *PostgresLifecycleStore) LastPolicyHash(ctx context.Context, _ string) (string, error) {
...
func (s *PostgresLifecycleStore) WriteCheckpoint(ctx context.Context, sessionID string, turnNumber int, t2HighWater string, tokenUsage int, state string) error {
	_, err := s.db.ExecContext(ctx,
...
func (s *PostgresLifecycleStore) InterruptStaleCheckpoints(ctx context.Context, cutoff time.Time) (int, error) {
...
func (s *PostgresLifecycleStore) LastCheckpointState(ctx context.Context) (string, error) {
	row := s.db.QueryRowContext(ctx,
...
func (s *PostgresLifecycleStore) LastWakeAt(ctx context.Context) (time.Time, error) {
	row := s.db.QueryRowContext(ctx,
