...
a warning but do not halt startup unless `STRICT_IDENTITY=true` is set.

## Integrity

SHA-256 anchor hashing is active. Each file's hash is stored at first boot;
...
enforces this check on fresh-identity first boot.

To bypass (CI / testing only): set `SKIP_WITNESS_CHECK=true`. This is logged explicitly
